...
class Node1:
    ...
    parent: Optional[Node1]
    block: Optional[Node1]
    incoming_non_tree: set[Transition]
    incoming_tree: set[Transition]
    depth: int

    def __init__(
        self,
        is_leaf: bool,
        children: tuple[Optional[Node1], Optional[Node1]] = (None, None),
        discriminator: str = ""
    ) -> None:
        ...
        self.parent = None
        self.block = None
        self.incoming_tree = set()
        self.incoming_non_tree = set()
        self.depth = 0  # TODO: set the depth properly
        ...
        if self.is_leaf:
            ...
            self.leaf = None
            self.inner = Inner(children, discriminator)

    @classmethod
    def make_leaf(cls) -> Node1:
        return Node1(True)
    ...
    def make_inner(
        ...
        new_node = Node1(False, children, discriminator)
    ...
    def __repr__(self) -> str:
        if self.is_leaf:
            assert self.leaf
            return f"Node<{self.leaf.state}>"
        else:
            assert self.inner
            if self.inner.is_temporary:
                return f"Node<~{self.inner.discriminator}>"
            else:
                return f"Node<{self.inner.discriminator}>"

    def get(self) -> Union[Leaf, Inner]:
        if self.is_leaf:
            assert self.leaf
            return self.leaf
        else:
    ...
    def print_tree(self, child: int = -1, level: int = 0):
        ...
        if self.is_leaf:
            ...
            assert inner.children[0]
            assert inner.children[1]
            inner.children[0].print_tree(0, level+1)
            print(f"{" " * 4 * level}{arrow} {inner}")
            inner.children[1].print_tree(1, level+1)

    def __iter__(self) -> Generator[Node1, None, None]:
        if self.is_leaf:
            yield self
        else:
            inner = self.get()
            assert isinstance(inner, Inner)
            if inner.children[0]:
                for node in inner.children[0]:
                    yield node
            yield self
            if inner.children[1]:
                for node in inner.children[1]:
                    yield node

    def states(self) -> Generator[State, None, None]:
        for node in self:
            if isinstance(leaf := node.get(), Leaf) and leaf.state is not None:
                yield leaf.state

    @classmethod
    def lca(cls, nodes: list[Node1]) -> Node1:
        min_depth = min(map(lambda node: node.depth, nodes))
        nodes_in_layer: set[Node1] = set()
...
class Inner:
    is_temporary: bool
    discriminator: str
    children: tuple[Optional[Node1], Optional[Node1]]
